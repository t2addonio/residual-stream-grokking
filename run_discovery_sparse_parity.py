#!/usr/bin/env python3
"""run_discovery_sparse_parity.py"""
import argparse
import json
...
RESULTS_DIR = Path("results_sparse_parity")
...
def make_sparse_parity_dataset(n_samples, seed):
...
def run_one(seed):
    print(f"\n===== Seed {seed} | sparse parity n={N_BITS} k={K_SPARSE} =====", flush=True)
    torch.manual_seed(seed)
    model = make_model().to(DEVICE)
    train_x, train_y = make_sparse_parity_dataset(N_TRAIN, seed)
    test_x, test_y = make_sparse_parity_dataset(N_TEST, seed + 10000)
    train_x, train_y = train_x.to(DEVICE), train_y.to(DEVICE)
    test_x, test_y = test_x.to(DEVICE), test_y.to(DEVICE)
    opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    result = {"seed": seed, "task": "sparse_parity", "n_bits": N_BITS, "k_sparse": K_SPARSE}
    timeline = []
    ...
    ckpt_path = RESULTS_DIR / f"grokked_parity_seed{seed}.pt"
...
def main():
    ...
    out = RESULTS_DIR / f"parity_{args.start}_{args.end}_{stamp}.json"
