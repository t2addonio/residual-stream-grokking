#!/usr/bin/env python3
"""
run_discovery_mult_L4_d512.py
Modular multiplication at d_model=512, 4 layers.
"""
...
RESULTS_DIR = Path("results_mult_L4_d512")
...
def run_one(seed, lr=LR, weight_decay=WEIGHT_DECAY):
    print(f"\n===== Seed {seed} | Mult L4 d=512 =====", flush=True)
    ...
    for step in range(MAX_STEPS):
        ...
        if step % 2000 == 0 or step == MAX_STEPS - 1:
            ...
            print(f"  step {step:5d}  test_loss={tloss:.4f}  acc={tacc:.3f}", flush=True)
    ...
    if not grokked:
        print("  Did not grok — skipping subspace & causal tests", flush=True)
        return result

    ckpt_path = RESULTS_DIR / f"grokked_mult_L4_d512_seed{seed}.pt"
    torch.save(model.state_dict(), ckpt_path)
    print(f"  Saved checkpoint → {ckpt_path}", flush=True)

    print("  Extracting subspaces...", flush=True)
    ...
    print(f"  Overlap (contrastive vs algorithmic) = {overlap:.5f}", flush=True)
    ...
    for name, B in [("mem", mem_B), ("algorithmic", algo_B), ("random", rand_B), ("union", union)]:
        ...
        for coeff in [0.0, 1.0, 2.0]:
            ...
            print(f"  {name:12s} c={coeff:.1f} → loss={loss:.4f} acc={acc:.3f}", flush=True)
...
def main():
    ...
    for seed in range(args.start, args.end + 1):
        ...
        out = RESULTS_DIR / f"mult_L4_d512_{args.start}_{args.end}_{stamp}.json"
        with open(out, "w") as f:
            json.dump(all_results, f, indent=2)

    print(f"\nFinished seeds {args.start}–{args.end}", flush=True)
