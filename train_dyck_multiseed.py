#!/usr/bin/env python3
"""
train_dyck_multiseed.py

Runs the full Dyck experiment across multiple seeds.
Prints and saves the complete phase-cancellation results.
"""
...
RESULTS_DIR = Path("results_dyck_multiseed")
...
def get_train_heldout(seed):
    ...
    x_ho, y_ho = make_dataset(2000, max_len=28, max_depth=5, seed=seed + 1000)
...
def run_seed(seed):
    ...
    print(f"Cancel contrastive (coeff=2):    {acc_contrast:.3f}")
    print(f"Cancel algorithmic (coeff=2):    {acc_algo:.3f}")
    print(f"Cancel random (coeff=2):         {acc_rand:.3f}")
    print(f"Overlap (contrastive vs algo):   {overlap:.4f}")
    ...
    # Save per-seed
    out = RESULTS_DIR / f"seed_{seed}.json"
    ...
    torch.save({
        ...
        "B_algo": B_algo.cpu(),
        "B_rand": B_rand.cpu(),
    }, RESULTS_DIR / f"subspaces_seed_{seed}.pt")
...
def main():
    ...
    print("="*60)
    print(f"{'Seed':<8} {'Base':>8} {'Contrast':>10} {'Algo':>8} {'Random':>8} {'Overlap':>8}")
    for r in all_results:
        print(f"{r['seed']:<8} {r['acc_base']:>8.3f} {r['acc_cancel_contrastive']:>10.3f} "
              f"{r['acc_cancel_algorithmic']:>8.3f} {r['acc_cancel_random']:>8.3f} {r['overlap_contrastive_algo']:>8.4f}")
