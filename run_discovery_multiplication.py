#!/usr/bin/env python3
"""
run_discovery_multiplication.py

Full discovery pipeline for modular multiplication (a * b mod p):
  train → contrastive subspace → algorithmic control → overlap → causal ablation
"""
...
RESULTS_DIR = Path("results_multiplication")
...
def construct_multiplicative_targets(x, k_max=8):
...
def extract_algorithmic_subspace(model, x, layer=LAYER, k_out=K_DIMS, k_max=8):
    ...
    n = min(4096, len(x))
    xb = x[:n].to(DEVICE)
    targets = construct_multiplicative_targets(xb, k_max=k_max)
...
def run_one(seed, d_model, lr=1e-3, weight_decay=1.0):
    print(f"\n===== Multiplication | Seed {seed} | d_model={d_model} =====")
    torch.manual_seed(seed)
    x, y = make_dataset()
    ...
    result = {
        "seed": seed, "d_model": d_model, "task": "multiplication",
    ...
    ckpt = RESULTS_DIR / f"grokked_mult_d{d_model}_seed{seed}.pt"
    ...
    union_B = torch.cat([mem_B, algo_B], dim=0)
    union_B = torch.linalg.qr(union_B.T, mode="reduced")[0].T
    rand_B = make_random_subspace(mem_B.shape[0] + algo_B.shape[0], d_model)

    causal = {}
    for name, B in [("mem", mem_B), ("algorithmic", algo_B), ("union", union_B), ("random", rand_B)]:
        causal[name] = {}
        for coeff in [0.0, 1.0, 2.0]:
            ...
            print(f"  {name:12s} c={coeff:.1f} → acc={acc:.3f}")
...
def main():
    ...
    for seed in range(args.start, args.end + 1):
        ...
        out = RESULTS_DIR / f"mult_d{args.d_model}_{args.start}_{args.end}_{stamp}.json"
