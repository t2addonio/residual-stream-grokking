#!/usr/bin/env python3
"""
logit_contribution_p257.py

Measure logit contribution of the contrastive ("mem") subspace
on already-trained p=257 modular-addition models (d_model=512, 2 layers).
"""
...
RESULTS_DIR = Path("results_logit_p257")
...
def run_one(checkpoint_path, seed):
    print(f"\n===== Logit contribution p=257 | seed {seed} =====")
    ...
    # simple complement of mem
    Q, _ = torch.linalg.qr(
        torch.cat([mem_B, torch.randn(D_MODEL - K_DIMS, D_MODEL, device=DEVICE)], dim=0).T
    )
...
def main():
    ...
    out = RESULTS_DIR / f"logit_p257_{stamp}.json"
