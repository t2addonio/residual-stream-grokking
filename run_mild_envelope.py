...
# Control
for seed in range(4000, 4015):  # 15 seeds
    results.append(run_with_weight_tracking(
        seed=seed, base_norm=999.0, use_envelope=False, name="Control"
    ))

# Static 2.0 (mild static baseline)
for seed in range(4100, 4115):  # 15 seeds
    results.append(run_with_weight_tracking(
        seed=seed, base_norm=2.0, use_envelope=False, name="Static 2.0"
    ))

# Mild Weight Envelope (2.0 → 1.6)
for seed in range(4200, 4215):  # 15 seeds
    results.append(run_with_weight_tracking(
        seed=seed,
        base_norm=2.0,
        use_envelope=True,
...
with open(f"results/mild_envelope_results_{timestamp}.json", "w") as f:
    json.dump(results, f, indent=2)

# Summary
print("\n" + "="*70)
print("MILD ENVELOPE SUMMARY")
print("="*70)
