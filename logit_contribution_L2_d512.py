#!/usr/bin/env python3
"""
logit_contribution_L2_d512.py

Measures logit contribution of the contrastive ("mem") subspace
on the existing 2-layer d_model=512 checkpoints.
"""
...
D_MLP = 2048
K_DIMS = 16
LAYER = 1                    # final residual stream for 2-layer model
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
RESULTS_DIR = Path("results_logit_L2_d512")
...
def run_one(checkpoint_path, seed):
    print(f"\n===== Logit contribution | seed {seed} | L=2 d=512 =====")
...
def main():
    ...
    parser.add_argument("--ckpt_dir", type=str, default="results_discovery")
    ...
    for seed in args.seeds:
        ...
        candidates = [
            Path(args.ckpt_dir) / f"grokked_d{D_MODEL}_seed{seed}.pt",
            Path(args.ckpt_dir) / f"grokked_d512_seed{seed}.pt",
            Path("results_discovery") / f"grokked_d512_seed{seed}.pt",
        ]
        ckpt = None
        for c in candidates:
            if c.exists():
    ...
    out = RESULTS_DIR / f"logit_L2_d512_{stamp}.json"
